...
public class AutorController {
    ...
    public ResponseEntity<SalvarAutorResponse> salvar(@RequestBody @Valid final SalvarAutorRequest autorRequest) {
        final Autor autor = autorRepository.save(autorRequest.getAutor());
    ...
    public ResponseEntity<ConsultarAutorResponse> consultarPorCodigo(@PathVariable String codigo) {
        final Optional<Autor> autor = autorRepository.findByCodigo(codigo);
        return ResponseEntity
                .of(autor.map(ConsultarAutorResponse::new));
    }
